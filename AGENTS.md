...
**当前阶段**：设计/文档已定稿；代码已进入实现阶段——V1 主链 段1→6→10→11 闭合，V1 任务包 M0–M12 各切片与 V2 P4 段12 首片（Q116）已落地；迁移头 0026、后端 472 测试、eval 101/101、前端 7 个 checker（CI 仍跑 6 个）。当前基线、待办与逐片记录一律以 [handoff.md](handoff.md)「当前状态/待办」与 `docs/02` C1 日志为准，本文件不复述数字。
...
**无原文挂起的待裁决事项；另有 4 项「实现差异」挂起待拍板**（截至 2026-09-17）：

- ~~两套路线图口径合并~~ 已于 2026-09-13 裁决（Q73，08 §1.3：A7 为骨 + D9 厚度，段 12 后置 V2，V1=段 1→6→10→11）。
- **2026-09-17 审计发现 4 项实现与文档不一致，Q117 只挂账未裁决**（详见 [handoff.md](handoff.md) 待办 6）：① 4 个 `/api/admin` GET 无 query actor 闸且不在 Q109 穷举清单内（content-goals / cp-law-domains / publish-slots / slot-type-defaults）；② `publish_slots.gate` 默认 `approved` 与 13 §1.9 `pending_gate` 口径不符；③ `product_spaces.category_node_id`/`active_pws_id` 无 DB 级 FK；④ `frontend/scripts/check-settings.mjs` 未接入 CI 与防漂移单测。**动这四处前必须先由负责人拍板并在 02 追加 Q 记录。**
