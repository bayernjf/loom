...
> **当前状态**：设计/文档已定稿（[`docs/`](docs/) 为项目**唯一事实源**）；**代码已进入实现阶段**——V1 主链 段1→6→10→11 闭合，M1–M12 各切片、V2 P4 段12 首片（Q116）、Q118 一致性收口（4 GET 补闸 / gate V1 口径回填 / 迁移 0027 两 FK / CI 七 checker）与 Q119 段12 多语言（content_languages 语言清单 / 市场∩产品目标语言交集 / 每语言独立成品，迁移 0028）已落地，迁移头 0028、后端 494 测试、eval 101/101。当前进度、待办与接手须知见 [handoff.md](handoff.md)。
...
- **可审计可回写**：Q1–Q118 决策日志、writeAudit、唯一出口、反馈回流闭环（唯一反向边回到段 3/4/8）。
...
| [docs/02_决策记录_ADR_Q1-Q72.md](docs/02_决策记录_ADR_Q1-Q72.md) | 决策日志 Q1–Q118 + 配置化清单 |
