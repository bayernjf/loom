...
async def test_mid_confidence_creates_ops_todo_and_sweep_escalates(client, session_factory):
    ...
    # 未到 72h 不升级。
    async with session_factory() as session:
        assert await modeling_service.escalate_due_todos(session) == 0

    # 超过 72h sweep 升级（Q4）。
    future = datetime.now(UTC) + timedelta(hours=73)
    async with session_factory() as session:
        assert await modeling_service.escalate_due_todos(session, now=future) == 1
...
async def test_c7_layers_and_fid_guard(client):
    ...
    r = await client.post("/api/categories", json={"name": "护肤"})
    parent = r.json()["category_id"]
    r = await client.post("/api/categories", json={"name": "面霜", "parent_id": parent})
    sibling = r.json()["category_id"]
    r = await client.put(
        f"/api/categories/{sibling}/template",
        json={"field_list": ["f_name", "f_brief"], "status": "approved", "actor": OPS},
    )
    assert r.status_code == 200, r.text
    ...
    r = await client.post("/api/categories", json={"name": "乳液", "parent_id": parent})
    ...
    r = await client.post("/api/categories", json={"name": "独立类目"})
    ...
    r = await client.put(
        ...
        json={"field_list": ["-"], "status": "draft", "actor": OPS},
    )
    assert r.status_code == 422
    r = await client.put(
        f"/api/categories/{new_cat}/template",
        json={"field_list": ["f_name", "ghost_fid"], "status": "draft", "actor": OPS},
    )
    assert r.status_code == 422
