...
**M10 切片 e · skill7 AI 候选通道（WF-04 试点；WF-02 字段池为首个复用切片，Q78）**（2026-09-14，迁移 0012，Q76/Q78；路由前缀 `/api`）

| 方法/路径 | 说明 | 依据 |
|---|---|---|
| POST `/skill-runs` | AI 产出外部投递（**operations**，越权 403；机器对机器 API Key 通道 05 §1.2【待补】）：body=skill_id/wf_id(可省，按注册表推导)/product_space_id/input/output/candidates[{target_type,payload}]/confidence/tokens/actor；未注册 Skill 或 Skill 不属该 WF → 422，PS 不存在 404；**投递校验按 WF 步骤声明的 `candidate_target` 泛化（Q78）**：非候选产出步骤带候选 422、target_type 与声明不符 422、payload 不符该 target 既有契约 422；写不可变 `skill_runs(status=succeeded,source=delivery)` + 每候选 `skill_candidates(pending_review)`，writeAudit `skill7.run_delivered` | Q76-1/2/5，Q78，06 §4 |
...
> 注册表：`runtime/workflows/WF-04.yaml`（顺序 Skill + 2 个 Gate 插槽）与 `runtime/skills/{PWC-BUILDER,COMBO-VALIDATE,PWC-SCORING}/skill.yaml`，以及 **WF-02 替换切片（Q78）**`runtime/workflows/WF-02.yaml`（FIELDPOOL-PLAN→DIM-SOURCE→DIM-MERGE，DIM-MERGE 步骤 `produces_candidates: true, candidate_target: field_plan`，skill7+fieldpool 双 Gate 插槽）与 `runtime/skills/{FIELDPOOL-PLAN,DIM-SOURCE,DIM-MERGE}/skill.yaml`（均 06 §4.1 的 10 字段；原文称 11 字段但逐项列出 10 项，按 10 项实现不擅补）；加载器 `app/core/skill7/registry.py`（`candidate_target_for(wf,skill)` 供投递校验），`LOOM_RUNTIME_DIR` 可覆盖路径。**仍挂账**：WF-01/03（M2 C1/C7、M4 原子批次）占位入参的通道替换按本切片同模式后续切片（每切片自带 eval/ 回归案例，Q77 模式）；Q67 模型注册表/LLM 客户端/API Key 鉴权随真接 LLM 切片；编排器并行/DAG 二期；多副本补货防抖锁随调度锁挂账。
...
> **实现补登（2026-09-14，M10 切片 e，Q76；WF-02 切片 Q78）**：通道落 `app/core/skill7/`；`ai_suggested` 为生产者侧态，外部投递落库即 `pending_review`，不持久化 ai_suggested 行【实现补】。confirmed/modified → 适配器应用后置 `applied`（不经过独立 confirmed/modified 持久态，二者体现在 human_modified 与审计动作上）；rejected → `archived`。已接入两个 target_type 适配器：`pwc_combo`（WF-04）复用既有 `pwc/funnel`，`field_plan`（WF-02，Q78）复用既有 `fieldpool.submit_plan`——AI 候选落库后仍走各自待 Gate→下游流程，skill7 不替代任何既有 Gate。投递侧契约校验按 WF 步骤 `candidate_target` 分派（Q78 起不再硬编码 pwc_combo）。
