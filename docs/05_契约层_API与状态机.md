...
**M10 切片 e · skill7 AI 候选通道（WF-04 试点；WF-02 字段池 Q78、WF-01 冷启动识别 Q79、WF-03 原子批次 Q80 为复用切片）**（2026-09-14，迁移 0012 + 0013，Q76/Q78/Q79/Q80；路由前缀 `/api`）
...
> **实现补登（2026-09-14，M10 切片 e，Q76；WF-02 切片 Q78；WF-01 切片 Q79；WF-03 切片 Q80）**：通道落 `app/core/skill7/`；`ai_suggested` 为生产者侧态，外部投递落库即 `pending_review`，不持久化 ai_suggested 行【实现补】。confirmed/modified → 适配器应用后置 `applied`（不经过独立 confirmed/modified 持久态，二者体现在 human_modified 与审计动作上）；rejected → `archived`。已接入四个 target_type 适配器：`pwc_combo`（WF-04）复用既有 `pwc/funnel`，`field_plan`（WF-02，Q78）复用既有 `fieldpool.submit_plan`，`c1_recognition`（WF-01，Q79）复用既有 `modeling.submit_recognition`，`atom_batch`（WF-03，Q80）复用既有 `atom.submit_batch`（强制 source=ai）——AI 候选落库后仍走各自待 Gate/三分支/逐条 approveAtomGuard→下游流程，skill7 不替代任何既有机械逻辑或 Gate。投递侧契约校验按 WF 步骤 `producer_step_for(...).candidate_target/single_candidate` 分派（Q78 起不再硬编码 pwc_combo；Q79 单候选约束声明化；Q80 atom_batch 复用同一整批单候选声明）。Q79-4 起 skill_runs/skill_candidates 双锚点（PS 或 intake，后者 product_space_id 改 nullable，迁移 0013；atom_batch 走 PS 锚点，无新迁移）。
