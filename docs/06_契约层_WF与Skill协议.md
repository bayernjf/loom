...
- **占位协议名单**：原文未逐条列出；基准 HTML 经 2026-09-17 确认永久丢失，Q115 收口——以 12 号文档「7 定稿 + 35 推断候选」为**最终名单**（35 候选保留【推断】标注，见 12 §1.2），不再从基准 HTML 的 PT 注册表导出。
...
- **实现状态（2026-09-15，M8 restock_auto worker，Q87；四接缝经负责人拍板记 02 C1.31，均选推荐项）**：Q71/Q76-4 的 `skill_runs(status=requested, source=restock_auto, skill=PWC-BUILDER)` 信号行由进程内第二个 asyncio 循环 `RestockWorker`（同构 SweepScheduler，V1 单进程；队列选型就此裁决为进程内轮询，Redis Streams/多副本锁后置 V2）消费。**SkillRun append-only 不放松**：requested 信号行永不 mutate——成功追加 `succeeded/llm_auto` 子 run（`input.restock_request_id` 回链，候选挂子 run）、终态失败追加 `failed/llm_auto` 子 run（Q71 预留的 failed 状态首次启用），认领=反连接已有子 run；瞬态（BudgetExhausted 日预算硬停/上游坏）不留子 run 下轮重试。范围仅 PWC-BUILDER 唯一信号写者，只产 pending_review 候选，product_reviewer Gate 不改；worker 走内部入口 `invoke_pwc_build_for_restock`（system:llm-gateway，OPERATIONS 闸仅存 HTTP 路径），不自动串 ATOM-EXPAND。开关 `LOOM_RESTOCK_WORKER_ENABLED` 默认 false（opt-in）、间隔 60s、每轮 20 条（env 实现值非 Q9 旋钮）；手工端点 `POST /api/admin/restock/run`（platform_admin）。终态=PS 消失/池未批/原子跨维<2/结构脏输出/配置缺失/停用无 fallback，audit 三动作 restock_delivered/restock_failed(terminal)/restock_deferred。无迁移（纯代码切片）。
- **实现状态（2026-09-15，Q67/Q86 WF-03 原子批量补池真 LLM 第五站/末站：chat + embedding 双调用）**：接缝四项经负责人拍板记 Q86（02 C1.30）：①触发=`POST /api/product-spaces/{id}/atom-batches/llm-expand`（operations，batch_size 缺省 Q14 敏感 20/非敏感 50），花 token 前预闸 PS/字段池 404、gate≠approved 409、Q15 已通过+冻结原子数 ≥ target_atom_max → 409 TargetReached（仅约束 AI 批次），不自动串链；②**一次触发两次调用、一个 atom_batch 候选**：CONFLICT-PRECHECK（WF-03 唯一 `candidate_target: atom_batch, single_candidate: true` 步骤，Prompt 挂此 skill）chat 产整批候选，ATOM-AFFINITY 为同 WF 的 embedding 步骤——**首个 capability=embedding 场景：不挂 Prompt、不产候选**，网关 `embed(scene, texts)` 路由能力不符 422；只落 1 条 SkillRun（skill_id=CONFLICT-PRECHECK，model_id=chat 模型，两次 input tokens/cost 求和、output 仅 chat，embedding 模型身份记 run.input 可审计）；候选原样复用 Q80 apply_atom_batch→submit_batch（强制 source=ai，Q14/Q15/批内去重/line 11189/Q17/line 840/Q18 一项不绕），1536 维向量以机读 `_embeddings` 键随候选 payload 带外传递（投递校验与适配器双侧 pop），approve 复制向量到 product_atom_instances；③Q16 相似度本体 V1：PG `vector(1536)`（迁移 0018 建 pgvector 扩展，SQLite 端 float32 LargeBinary TypeDecorator），进程内纯 Python 余弦 max 取同维 approved/frozen 有向量原子（无基 None，Q16 低亲和 0.5 只标不淘汰），批内同维并查集成簇线 `atom.cluster_line=0.9`（借 Q10 同义线，原文未给【待补】，进配置中心），KNN 索引/`<=>` 千万级切换按 14 §2.3 后置；④输出白名单 `{batch_size 回传, items:[{content, dimension_id, ai_risk, evidence?, fact_type?}]}`，模型自报 affinity/cluster_id/score 一律剥离本地重算，**ai_risk 属 PT-ATOM-EXP/Q17 契约内 AI 字段枚举校验后透传**（词表只升不降仍在 submit_batch），结构脏数据/embedding 行数·维度不符投递前 502 不留痕，业务违规不 502；预算/失败沿用 Q82–Q85（409/422/502/disabled 已注册 fallback），系统 Actor `system:llm-gateway` roles=[]。**Q86 后五个真 LLM 站点全部接通，Q67 站点 backlog 闭合**；restock_auto 自动消费由 Q87 M8 worker 闭环（见下条）。eval 不新增（synthetic chat/embedding 构造器均为夹具）。
...
> **实现状态（2026-09-15，编排并行/DAG 原语，Q91；14 §2.2“复杂 DAG 二期”提前落地形态；02 C1.35 四接缝负责人全按推荐甲拍板）**：新包 `app/core/orchestration/`（planner.py 纯规划 + engine.py 异步执行）。**①YAML 声明**：步骤新增可选 `needs: [skill_id,...]`；全部步骤不声明 needs 时退化为声明序顺序链——WF-01..04 一字不改即可规划；显式 DAG 经 Kahn 拓扑分层（同层可并发、层间屏障），加载期校验拒绝未注册 skill 引用/未知 needs/依赖环/重复步骤/gate 锚点缺失。**②skill7 Gate 硬执行边界**：只规划第一道 channel=skill7 gate 的 after 步骤及其全部前驱（WF-01 仅 PARSE/UNDERSTAND/CAT-RECOG，第二道 gate 的 TYPE-MATCH 不进本轮；WF-04 仅 PWC-BUILDER；WF-03 至 CONFLICT-PRECHECK），引擎不跨人工 Gate（Q66 铁律）。**③执行**：同层 asyncio 并发受信号量 `LOOM_ORCH_MAX_CONCURRENCY`（默认 4，env 运维参数非 Q9 旋钮）约束；任一节点失败 fail-fast 取消同层兄弟、整轮失败（NodeExecutionError 携带失败节点与已完成节点），层间屏障保证后层读到全部前驱结果；引擎不自动重试（重试归调用侧，如 Q90 游标）。**④处理器注入、不记账不建表**：引擎自身不调模型/不投递候选/不写 skill_runs/无迁移；节点处理器按 skill_id 注入，同轮关联经 NodeContext.orch_run_id（uuid hex）+wf_id，真实站点接入时写 run.input。**本切片不接真实业务流**：五站点显式端点口径不变（Q84 CAT-RECOG→TYPE-MATCH 不自动串链维持），DIM-SOURCE 七路 fan-out 连接器 V2 来时声明 needs 即接入。测试 351→371（+20 单测 test_orchestration_dag.py），eval 不新增（注册表/调度基础设施，同 Q89/Q90 口径）仍 101/101；复杂 DAG 失控迁 LangGraph 的回滚口径不变（14 §2.2）。

### 4.3 Skill 调用日志（M6，D3.10 + 段10）
- 每次输入/输出/成本/置信度/失败/人工修改记录
- **历史 SkillRunLog 不可 mutate**（即便回滚，PT-COMPLIANCE-V2.0）
- **Q87 补登**：`requested`（Q71 补货信号）行同样不可 mutate；worker 以追加 succeeded/failed 子 run（`input.restock_request_id` 回链）表达认领与终态，从不更新信号行。
...
> **信息保全**：本文档所有 WF/Skill/PT/Agent/Gate 条目来自 v3 Part A（A1/A3/A4）与 Part D（D3/D6）原文；34 个占位协议名单原文未列出（Q115 收口：以 12 号文档「7 定稿 + 35 推断候选」为最终名单，35 候选保留【推断】标注，未编造协议内容）。展示口径 Skill 与权威口径 Skill 的对应映射原文未给出【待补】。
