...
| 接口 | 方法/路径 | 来源决策 | 契约状态 |
|---|---|---|---|
| 效果回调 | POST /api/effect-callback | Q60 | 🟢 契约定稿（见 §2.1）；**入站与操作面已落地（Q126–Q131，2026-09-19/20）**：Bearer 验签 + effect_records 时序 + matched/orphan 分流 + 运营只读队列 + 单条/批量认领/解绑（Q127/Q129）+ 客户专用回填通道（Q128）+ 管理端认领台与客户回填岛（Q130/Q131）+ 服务端 CSV 批量上传逐行回执（Q156）、Excel .xlsx 服务端上传（Q160）、异步导入任务三端点（Q161，worker 门控默认关、关走请求内同步）；反哺校准算法口径【待补】随 V2 |
...
- **Q156 服务端 CSV 批量上传（2026-09-21，02 C1.100，零迁移/零新依赖；三接缝（服务端强制 tz-aware／行数硬上限默认 1 万／CSV 走 JSON body 不引 multipart）**已经负责人 2026-09-22 追认销账，见 02 C1.100【追认（2026-09-22）】条**）**：`POST /api/effects/backfill/upload`，**无 Agent Key**（客户通道，同 Q128），JSON body `{tenant_id, content_id, csv, filename?, actor}`（不引 multipart，CSV 为文本字段；整份挂单一 content_id，九列同 Q136：platform_post_id,captured_at,plays,likes,comments,shares,inquiries,conversions,read_rate）。服务端标准库 csv 解析：去 BOM、表头按名定位（重复/未知/缺列聚合报错）、跳纯空行、回执物理行号（首数据行 line=2）、captured_at **强制带时区**（naive/纯日期逐行 422，守 Q128 tz-aware 铁律）、计数 `^\d+$`、read_rate 0..1、空指标缺席不落 0；**逐行收集全部坏行一次回 422**（detail `{message:"CSV validation failed", errors:[{index,line,field,message}]}`）；行数硬上限 env `LOOM_BACKFILL_UPLOAD_MAX_ROWS` 默认 10000（0 数据行/超限 422）。通过后复用 Q128 客户通道（只命中本租户非 discarded 成品否则整批 422、绝不孤儿、all-or-nothing、审计 effect.customer_backfilled），200 回执在 `{received,matched,orphan,upserted}` 上追加 `filename` 与 `rows[]{index,line,platform_post_id,captured_at}`。~~Excel（openpyxl）解析~~（已随 Q160 落地，见下）、~~异步导入任务~~（已随 Q161 落地，见下）；~~前端岛切异步任务轮询~~（已随 Q174 落地，见下）。
- **Q160 客户批量回填 Excel（.xlsx）服务端上传（2026-09-22，02 C1.104，零迁移/加 openpyxl 依赖，甲案接缝已经负责人追认）**：`POST /api/effects/backfill/upload-excel`，**无 Agent Key**（客户通道，同 Q128/Q156），与 CSV `/upload` 同契约、同回执 `CustomerBackfillUploadReceipt`；JSON body `{tenant_id, content_id, content_base64, filename?, actor}`（`content_base64` 为 .xlsx 字节的**标准 base64**，JSON body 携文本、零 multipart）。新模块 `app/core/effects/excel_io.py`：openpyxl `read_only` 流式读**活动工作表**（V1 只认第一个 sheet），单元格值归一化为字符串后交 `csv_io.parse_backfill_table` 做与 CSV **完全一致**的严格校验（9 列表头按名定位、逐行收集坏行、all-or-nothing、绝不孤儿）。归一化：空单元格→""（指标缺席绝不补 0）、整数/整数值浮点（100.0）→整数字面量、布尔→true/false、日期单元格 `isoformat()` 不带时区→naive ISO 串被 tz-aware 铁律逐行拒绝（不臆造客户时区）、尾部空列补齐到 9（仅第 10 列后非空才报多列）；旧 .xls（BIFF）/CSV/损坏文件在加载期即转 `CsvValidationError(field="file")` 由路由回 **422**（detail `{message:"Excel validation failed", errors:[...]}`）。行数硬上限复用 env `LOOM_BACKFILL_UPLOAD_MAX_ROWS` 默认 10000；审计复用 `effect.customer_backfilled`；解析通过后复用 Q128 ingest。依赖 pyproject `openpyxl>=3.1.5`。
- **Q161 客户批量回填异步导入任务（2026-09-22，02 C1.105，迁移 0039 import_jobs、业务物理表 60→61，甲案接缝已经负责人追认）**：新横切包 `app/core/imports/`（models/router/schemas/service/worker），端点挂效果回填命名空间 prefix `/api/effects/backfill`：①`POST /jobs`（**201** `BackfillImportJobView`，body `BackfillImportJobIn{tenant_id, content_id, format: csv|xlsx=csv, csv? | content_base64?+filename?, actor}`，载体与 payload 不匹配/二者皆空 422）；②`GET /jobs?tenant_id（必填 min_length=1）&content_id?&limit=50（1..200）`→envelope `{tenant_id,count,jobs[]}`（按租户倒序、可选成品过滤，**列表不含 payload 大字段**）；③`GET /jobs/{job_id}?tenant_id=`（租户隔离状态轮询 queued/running/completed/failed，未知或越权 **404** `import job not found`）。`import_jobs.payload` **Text 落 CSV 原文/xlsx base64 可重放**（导入是写，刻意相反于 Q132 export_jobs 的不存 payload、下载按参数重渲染），五计数 received/matched/orphan/upserted/row_count，确定性校验坏行落 `errors`（JSON 截断）。**门控 `LOOM_IMPORT_WORKER_ENABLED` 默认 false 保 V1 单副本行为**：关＝POST 先建 queued 行提交、随即请求内同步跑到终态再提交，**201 直接回 completed/failed**（逐行坏行在 errors）；开＝queued 提交后 XADD 由 `ImportWorker` 异步置 running→completed/failed，**入流基础设施失败 fail-closed 置 failed 回 503** detail `import queue unavailable; job marked failed`（不留无人消费的 queued）。worker 经 Redis Streams 消费组（镜像 Q137/Q152：read_new delivery=1、reclaim_pending 崩溃接管、成功 ACK、不可见未超限留 PEL、超限 move_to_dead 置 failed、终态重复幂等 ACK）；**确定性校验失败（CsvValidationError/EffectValidationError）直接 failed+ACK，不重试不进死信**（坏数据重试无意义），仅基础设施异常留 PEL/死信。另三 env（运维旋钮、按 Q135 先例不入 backend/.env.example/C2）：`LOOM_IMPORT_STREAM_BLOCK_SECONDS=5.0`、`LOOM_IMPORT_WORKER_CONCURRENCY=1`、`LOOM_IMPORT_STREAM_COUNT=20`。审计 `import.job_created`/`import.job_completed`/`import.job_failed`。**前端岛已随 Q174 切异步任务轮询**（批量回填/Excel 提交 `POST /jobs` → 轮询 `GET /jobs/{id}` → 渲染终态；门控关时提交即 completed/failed，门控开时 queued 轮询）。
- **未含（随下一片/V2）**：效果反哺/Q54·Q57 校准算法（口径【待补】）、~~回填批量表格/CSV 导入~~（CSV 已随 Q156、Excel 随 Q160、异步任务随 Q161、前端岛异步轮询随 Q174 全部落地）、content_id↔platform_post_id 映射自助化、Agent 抓取与发布自动化（Q62 不抓取）。~~认领取消/解绑与认领/回填前端 UI~~（已随 Q129–Q131 落地）。
