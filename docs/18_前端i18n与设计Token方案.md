...
3. 建 messages/zh-CN.json，现有页面文案全部改 `useTranslations`/`getTranslations`；命名空间按后端域：`common.*`、`nav.*`（Q97 八菜单）、`shell.*`（Q97 外壳/占位）、`products.*`（Q98 产品中心）、`workbench.*`（Q99 工作台）、`intake.*`（含 `intake.status.*` 十五态，Q99 落地，与 docs/13 状态词一一对应）、`fieldpool.*`、`pwc.*`、`pws.*`、`skill7.*`（候选/裁决/工作台）、`dashboard.*`、`tenant.*`（Q95 租户管理/引导）、`error.*`（错误码映射）；
...
| 模板管理 | `/templates` | **v2**（模板库随 V2 类目，Q114 改 v2 徽标占位） |
| 数据分析 | `/analytics` | **v2**（段13） |
| 社媒账号 | `/social-accounts` | **v2**（段8/发布账号完整版） |
| 合规风控 | `/compliance` | v1（段10 侧） |
| 系统设置 | `/settings` | v1（只读账户面板+语言展示，Q114；通知/帮助/团队/账单挂 V2 徽标） |

- **占位口径**：v1 页文案 `shell.placeholderV1`（随 M12 后续切片落地），v2 页标题旁与导航项均挂"V2"徽标 + `shell.placeholderV2`；菜单全显不隐藏。
- **管理端分治**：~~未来落 `/[locale]/admin/*`~~ **已落地**（Q102 起，现 12 项 sidebar，含 Q130 效果台/Q167 Agent Key 治理／Q168 导出任务／Q177 白名单卡片台／Q178 运营令牌与 `/admin/login`，由 `check-admin` 守卫锁项数与文件集）；独立布局不与客户外壳共用导航这一点原样成立（客户 nav 恰 8 项，另一守卫）。**角色显隐**：Q178 门控开时后端 401/403 为准，前端仍不做按角色隐藏菜单（真实认证＝内部 PAT 已有、客户侧认证仍 V2，口径 C 甲）。
...
- **五指标卡**（D5 原文：今日生成/发布/互动/趋势/健康度）：V1 一律禁用态——V2 徽标 + “V2 随内容生产与反馈回流开放”，**不渲染任何数字（包括 0）**；数据源属段 12/13，V2 开放。
- **状态词**：`intake.status.*` 十五键与 docs/13 §1.1 一一对应（枚举码不翻译）；Q98 产品列表/详情状态 chip 同一切片切到该命名空间，§7“状态在 V1 直接显示后端英文码”临时口径收口。
- **快捷入口**：新建产品（`/products/new`）、查看全部产品（`/products`）。
- **漂移防护**：`scripts/check-workbench.mjs`（npm run check-workbench）校验 workbench.*/intake.status.* 必备消息键、页面与样式文件、force-dynamic、五卡指标名、后端 router 中 `/overview` 注册早于 `/{intake_id}`（跨栈守卫）；check-products.mjs 同切片增裸 `{intake.status}` 渲染守卫。失败均 exit 1。
