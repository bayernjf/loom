...
| [18_前端i18n与设计Token方案.md](./18_前端i18n与设计Token方案.md) | 14/09 + Q58/Q73 → 前端工程 | 🟡 工程建议（待负责人确认；确认后升 ✅ 并追加 Q75）：界面 i18n（next-intl 建议，V1 仅 zh-CN）+ 设计 Token 三层模型（CSS 变量，V1 CSS Modules） | M12 前端落地前需拍板 |
