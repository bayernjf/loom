...
> **状态**：✅ **已拍板并落地（2026-09-25，Q196／02 C1.140）**——负责人「好的，按你推荐来」＝**A 甲＋B 甲＋C 甲**，落地形态见 02 该条：审计身份在唯一口 `append_audit` 引擎级收口（凭证 > 自报，`detail._actor_via` 标来源）、客户回填审计租户取自成品行、导出任务两条读口补 `tenant_id` 归属并按任务行收口、任务 ID 改 uuid4。本文正文（含十条实测与三问选项）**保留为裁决依据原文不改**，以下为落地时的选择记录：
> - **A 甲** 落地为「派生＋交叉校验」而非改端点签名：合法调用的响应一字未变；派生不变量由一条 monkeypatch 用例钉住（正常用例恒等、证不到差异）。
> - **B 甲** 落地在唯一审计口而非逐个端点（122 个调用点零改动），刻意不选 B 乙（92 个模型删 `actor` 属破坏性契约变更）。
> - **C 甲** 意味着本切片**没有**引入任何客户凭证形态；§1 事实 7 列出的客户读口仍以声明 `tenant_id` 过滤，这是 C 甲的**已接受风险**、不是遗漏，已在 02 C1.140「没闭合什么」条写明。
> - 未选路径：#31 的两条口**不加内部角色闸**（Q168 记「中台无 RBAC 闸」、轮询方是外部中台），改用租户归属收口——接缝③**已经负责人 2026-09-25「追认」销账（Q197/02 C1.141）**。
>
> 原文（拍板前）：⬜ **待负责人拍板**，本文档零代码零迁移、不作任何裁决。Q178（02 C1.122）落了身份层**第一切片＝内部运营人员 PAT**；本文件把**第二切片必须先定的三个口径**摆开，并给出**用命令实测出来的改造爆炸半径**，供拍板时估代价。拍板后按先例追加 Q 编号再落码。
>
