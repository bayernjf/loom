...
- **已定稿事实基线**：Q1–Q72 均已有决策记录（02，含正式采纳与"临时采纳"两类）；S 级红旗 4 条经 Q34/Q60 等解决（02 收官注、03 已标销账），**M0 收尾项 = Q2/Q22（C1 信号权重、图像信号提取、PWC 品类乘数公式）与 Q60（回流指标降级口径）由临时采纳转定稿**，另需补写 04 §3 段 1-6 实体缺口（详见 08 §2.1 M0，2026-09-13 已按 02/03 核对订正）；业务方 3 份素材已采纳并销账（D5→Q21、F3→Q70+Q49、E1→Q71+Q24），待向负责人明确告知留确认；两套路线图（A7 三阶段 vs D9 V1/V2/V3）口径冲突**原文标注待裁决**（08 §1.3）；技术选型 6 项建议待拍板（14）。
...
1. **M0 规格仲裁收尾（写代码前，08 §2.1，2026-09-13 已按 02/03 核对）**：
   - 待拍板转定稿：**Q2/Q22**——C1 五信号权重与图像信号提取方式、PWC 品类乘数公式（现临时采纳；08 M5 已引用"评分=(合理性0.6+多样性0.4)×品类乘数"）；**Q60**——回流指标降级具体口径（平台公开互动数指标集 + 客户手工回填转化）。
   - 规格补写（非裁决）：04 §3 段 1-6 未定义字段实体补齐（同步 10/12）。
   - 已无阻塞、仅留确认：PLATFORM-ADAPTER 协议已定稿（06 §2.3）；KUP-PROPOSE 属第二阶段段13，不阻塞 MVP；业务方 3 份素材已采纳销账（Q21/Q70+Q49/Q71+Q24），需向负责人明确告知并留确认记录。
