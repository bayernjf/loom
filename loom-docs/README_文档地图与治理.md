...
| [02_决策记录_ADR_Q1-Q72.md](./02_决策记录_ADR_Q1-Q72.md) | Part C 全量（C1 + C2） | 决策日志：Q1–Q72 全部定稿记录 + 配置化清单 | 核心 |
...
| [08_迭代计划与任务包.md](./08_迭代计划与任务包.md) | Part A7 + Part D9 | 路线图（两套口径并存）+ MVP/阶段任务包 | 中优先 |
...
| [14_技术选型决策.md](./14_技术选型决策.md) | 07 §7 工程建议 → 决策 | 🟢 已补：6 项建议方案+理由+风险（**待拍板**） | 待用户拍板 |
| [15_代码结构与脚手架.md](./15_代码结构与脚手架.md) | 07 架构 → 代码落位 | 🟢 已补：建议目录/模块/可执行物落位 | 待 14 拍板 |
| [16_测试策略.md](./16_测试策略.md) | Guard/Q/契约 → 测试计划 | 🟢 已补：分层测试 + 规则级测试清单 | 与 12 联动 |
| [17_部署与运维.md](./17_部署与运维.md) | 07/09 §11 → 运维 | 🟢 已补：环境/CI-CD/多租户/监控说明 | 待 14 拍板 |

> 原始文档：v3.md 已于 2026-09-13 归档删除（内容 100% 迁入本库，见 §5.3 核验记录）；HTML V2.0 保留在上级目录，仅作展示导航基准。

---

## 2. 文档治理规则（源自 v3 Part E2，全量保留；v3 删除后适配为本库规则）

1. **唯一事实源**：本目录（loom-docs/）是项目 Loom 唯一的权威文档体系；所有原型 / PRD / 开发任务 / Skill 设计均以本库为准。原 v3.md 已于 2026-09-13 归档删除，其内容 100% 迁入本库（核验见 §5.3）。
...
4. **两套口径**：Part A（13 段链，权威）与 Part D（03-A~E，展示）并存不混用；冲突时以 Part A 为准；口径合并属业务决策，需用户裁决后另行修订（裁决事项见 08 §1.3）。
...
5. **待裁决事项**（原文明确挂起，本库不擅自裁决）：① A7 三阶段 × D9 V1/V2/V3 路线图口径合并（见 08）；② 业务方 3 份素材是否采纳（见 03）；③ 技术栈选型等工程建议（见 07 §末 → 14 决策文档）。
